...
2. OSINT e Coleta de Inteligência (`hyper_osint`)
    ...
    A. `api_keys_found.txt` (Caso chaves sensíveis sejam encontradas no escopo).

    B. `hashes_found.txt` (Hashes extraídos do alvo).
    ...
    C. Após a varredura completa, o dicionário global consolidado é gravado no arquivo `relatorio_auditoria.json`.

    D. Estrutura interna salva no JSON:
    ...
    E. O arquivo `relatorio_auditoria.html` gerado automaticamente agrupa as informações de segurança em blocos de cartões estilizados para facilitar a leitura em qualquer navegador web moderno.
...
## ⚙️ Configuração
  ...
  PROXIES = [
      ...
      # Exemplo de Proxy HTTP: "http://127.0.0.1:8080"
      # Exemplo de Proxy Tor local: "socks5://127.0.0.1:9050"
  ]
  ```
